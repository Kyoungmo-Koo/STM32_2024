...
int main(int argc, char **argv)
{
    ...
    status = SPI_Write(handle, &tx_buffer[0], 4, ptransferCount, options);
    for (int i = 0; i < 100; i++) {
        start = clock();
        ...
        Sleep(1);
        status = SPI_Read(handle, &rx_buffer[0], 10001, ptransferCount, options);
        Sleep(1);
        if (rx_buffer[0] == 0) {
