...
uint32_t txData[10000];
uint32_t rxData[10000];
uint8_t buf[12];
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart){
	if(transmitFlag == 2 | transmitFlag == 6){
		HAL_UART_Receive_DMA(&huart1, txData, 5000 * sizeof(uint32_t));
	}
	else if(transmitFlag == 0 | transmitFlag == 4){
		HAL_UART_Receive_DMA(&huart1, &txData[5000], 5000 * sizeof(uint32_t));
	}
}
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  if(transmitFlag == 1){
		  ...
		  HAL_UART_Transmit_DMA(&huart1, flag_buffer, strlen((char*)flag_buffer));
	  }
